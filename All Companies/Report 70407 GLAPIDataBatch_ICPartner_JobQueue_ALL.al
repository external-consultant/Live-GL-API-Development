report 70407 GLAPIDataBatch_ICPartnerJQ_ALL//Job Queue 01-05-2025
{
    // UsageCategory = ReportsAndAnalysis;
    // ApplicationArea = All;
    ProcessingOnly = true;
    Caption = 'GLAPIDataBatch_ICPartner_JobQueue_ALL';
    Description = 'T52067-DataPerCom_ALL';
...
#pragma warning restore AL0254
                ...
                begin
                    ...
                    if "G/L Entry"."Source Type" = "G/L Entry"."Source Type"::Customer then begin
                        if Customer_gRec.Get("G/L Entry"."Source No.") then
                ...
                begin
                    GlActPipe_lTxt := '';
                    GlAct_gRec.RESET;
                    GlAct_gRec.ChangeCompany(Company.Name);
                    GlAct_gRec.SetRange("IC Partner Report Bre Act", TRUE);
                    IF GlAct_gRec.FindSet() Then begin
                        repeat
                            IF GlActPipe_lTxt = '' then
                                GlActPipe_lTxt := GlAct_gRec."No."
                            Else
                                GlActPipe_lTxt += '|' + GlAct_gRec."No.";

                        until GlAct_gRec.Next() = 0;
                    end;
    ...
    var
        ...
        GLAPIData_gRec: Record GLAPIData_ALL;
        StartTime: DateTime;
        EndTime: DateTime;
        Duration: Duration;
        GlAct_gRec: Record "G/L Account";
